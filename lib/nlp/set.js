...
export default {
  setString(text) {
    ...
    if (!words.includes(object)) {
      return util.handleError(returnObj, "Error: Unspecified variable type.");
    }
    switch (command) {
      case "assign":
      case "set":
  ...
  setDict(text) {
    ...
    switch (command) {
      ...
      if (!words.includes(launchPhrasePreposition)) {
        return util.handleError(returnObj, "Error: Invalid launch phrase preposition.");
      }
      if (words[words.indexOf(object)+2] === launchPhrasePreposition) {
        variable = words[words.indexOf(object)+1];
      } else {
  ...
  setNumber(text) {
    let returnObj = {error: false};
    if (!util.hasNumber(text)) {
      return util.handleError(returnObj, "Error: No number given.");
    }
    let words = text.split(/[ ,]+/);
    if (words.length != 4) {
      return util.handleError(returnObj, "Error: Your command is too complicated.");
    }
    let launchPhrasePreposition = "to";
